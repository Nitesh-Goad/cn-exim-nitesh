...
def update_rodtep(name, use_rodtep):
    ...
    doc = frappe.get_doc("Rodtep Utilization", name)

    used = float(use_rodtep or 0)

    # If remaining not set, use total amount
    current_remaining = float(doc.remaining_amount or doc.amount)

    new_remaining = current_remaining - used

    # Prevent negative
    if new_remaining < 0:
        frappe.throw(f"Rodtep {name} overused. Remaining: {current_remaining}, Trying to use: {used}")
