...
frappe.ui.form.on("Pre Alert", {
    ...
    before_save: function (frm) {
        validate_rodtep_assignments(frm);
        freight_amt_calculation(frm)
        insurance_calculation(frm, frm.doc.insurance_amount || 0);
        other_charges_calculation(frm)
        calculation_of_rodtep(frm)
        calculation_used_rodtep(frm)
        calculation_tax(frm)
            ...
            frm.add_custom_button("Pre-Check List", function () {
                ...
                frm.doc.item_details.forEach(item => {
                    items.push({
                        ...
                        'swl_': item.swl_,
                        'igst_': item.igst_,
                        'rodtep_utilization': item.rodtep_utilization,
                        'total_duty_forgone': item.total_duty_forgone,
                        'bcd_amount': item.bcd_amount,
                        'hcs_amount': item.hcs_amount,
                        'swl_amount': item.swl_amount,
                        'igst_amount': item.igst_amount
        ...
        frm.add_custom_button("Rodtep", function () {
            let d = new frappe.ui.form.MultiSelectDialog({
                ...
                action: function (selections) {
                    ...
                    process_rodtep_selections(frm, selections);
                }
            });
...
function calculation_of_rodtep(frm) {
    // reset item values
    (frm.doc.item_details || []).forEach(item => {
        item.rodtep_utilization = 0;
        item.total_duty_forgone = flt(item.bcd_amount);
    });

    // reset rodtep row values
    (frm.doc.rodtep_details || []).forEach(r => {
        r.used_rodtep = 0;
        r.custom_balance_amount = flt(r.custom_assigned_total_amount || 0);
    });

    // apply assigned rodtep row by row
    (frm.doc.rodtep_details || []).forEach(r => {
        let item = get_item_by_row_name(frm, r.custom_selected_item_row);

        if (!item) return;
        if (!item.custom_apply_rodtep) return;

        let available = flt(r.custom_assigned_total_amount || 0);
        let needed = flt(item.total_duty_forgone || 0);

        if (available <= 0 || needed <= 0) {
            r.used_rodtep = 0;
            r.custom_balance_amount = available;
            return;
        }

        if (available >= needed) {
            item.rodtep_utilization += needed;
            item.total_duty_forgone = 0;
            r.used_rodtep = needed;
            r.custom_balance_amount = available - needed;
        } else {
            item.rodtep_utilization += available;
            item.total_duty_forgone = needed - available;
            r.used_rodtep = available;
            r.custom_balance_amount = 0;
        }
    });

    let total_remaining = 0;
    (frm.doc.rodtep_details || []).forEach(r => {
        total_remaining += flt(r.custom_balance_amount || 0);
    });

    frm.set_value("rem_rodtep", total_remaining);
...
function calculation_used_rodtep(frm) {
    let total_rodtep_used = 0;

    (frm.doc.rodtep_details || []).forEach(r => {
        total_rodtep_used += flt(r.used_rodtep || 0);
    });

    frm.set_value("tot_rodt_ut", total_rodtep_used);
    frm.refresh_field("rodtep_details");
}
...
function update_rodtep_base_on_used(frm) {
    ...
    // Step 1: aggregate used rodtep per script
    (frm.doc.rodtep_details || []).forEach(d => {

        if (!d.script_no) return;

        let used = flt(d.used_rodtep || 0);

        if (used <= 0) return;

        if (!script_map[d.script_no]) {
            script_map[d.script_no] = 0;
        }

        script_map[d.script_no] += used;
    });

    // Step 2: call API once per script
    Object.keys(script_map).forEach(script => {

        frappe.call({
            method: "cn_exim.cn_exim.doctype.pre_alert.pre_alert.update_rodtep",
            args: {
                name: script,
                use_rodtep: script_map[script]
            }
        });
