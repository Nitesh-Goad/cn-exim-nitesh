    ...
    refresh: function (frm) {
        if (frm.doc.docstatus == 1) {
            ...
            frm.add_custom_button("PO Condition Change", function () {
                ...
                if (frm.__creating_po_condition) return;
                    frm.__creating_po_condition = true;
                    ...
                    frappe.call({
                        ...
                        callback: function (r) {
                            ...
                            frm.__creating_po_condition = false;
                            ...
                            let data = r.message[0] || [];
                            let parent_data = r.message[1] || [];

                            //  No items
                            if (data.length === 0) {
                                frappe.msgprint("No items found for this Pre Alert Check List");
                                return;
                            }

                            // 🔒 Safe parent access
                            let parent = parent_data.length ? parent_data[0] : {};
                            ...
                            data.forEach(item => {
                                ...
                                //  Skip broken rows
                                if (!item.item_code || !item.quantity) return;
                                ...
                                items.push({
                                    ...
                                    total_inr_value: item.total_amount || 0,
                                    currency: parent.currency || frm.doc.currency,
                                    rate_inr: item.quantity ? (item.total_amount / item.quantity) : 0
                                });
                            });

                            // After filtering, no valid items
                            if (items.length === 0) {
                                frappe.msgprint("No valid items to create PO Condition Change");
                                return;
                            }
                            ...
                            frappe.call({
                                ...
                                args: {
                                    doc: {
                                        ...
                                        vendor: parent.vendor || frm.doc.vendor,
                                        vendor_name: frm.doc.custom_vendor_name,

                                        cha_vendor: frm.doc.cha,
                                        cha_name: frm.doc.custom_cha_name,

                                        exchange_rate: frm.doc.exchange_rate || 0,

                                        bcd_amt: frm.doc.bcd_amount || 0,
                                        h_cess_amt: frm.doc.h_cess_amount || 0,
                                        sws_amt: frm.doc.sws_amount || 0,
                                        igst_amt: frm.doc.igst_amount || 0,
                                        total_duty: frm.doc.total_duty || 0,

                                        freight_amt: frm.doc.total_freight || 0,
                                        other_chrg: frm.doc.other_charges || 0,
                                        total_inr_val: frm.doc.total_inr_value || 0,
                                        accessible_val: frm.doc.accessible_value || 0,

                                        check_list_date: frm.doc.check_list_date,
                                        job_number: frm.doc.job_number,
                                        ad_code: frm.doc.ad_code,
                                        penalty: frm.doc.penalty || 0,

                                        // 🔹 CHILD TABLE (CONFIRM FIELDNAME)
                                        items: items
                                    }
                                },
                                callback: function (res) {
                                    if (res.message) {
                                        frappe.set_route("Form", "PO Condition Change", res.message.name);
                                    } else {
                                        frappe.msgprint("Failed to create PO Condition Change");
                                    }
                                }
                            });
                        ...
                        error: function () {
                            frm.__creating_po_condition = false;
                            frappe.msgprint("Server error while fetching items");
                        }
                    });
