...
//                       !response.message[0]["supplier"]
//                     ) {
//                       frappe.msgprint({
//                         title: __("Error"),
//                         indicator: "red",
//                         message: __(
//                           "No Supplier Quotation found for this RFQ and Pickup Request. Please create a Supplier Quotation first.",
//                         ),
//                       });
//                       return;
//                     }
...
//           doc.custom_port_of_loading = frm.doc.pol;
//           // doc.custom_mode_of_shipment = frm.doc.type_wise_value || 0;
//           doc.custom_mode_of_shipment = frm.doc.mode_of_shipment;
//           doc.custom_pickup_supplier = frm.doc.name_of_supplier;
...
//             doc.custom_dimension_calculation = [];

//             frm.doc.dimension_calculation.forEach((row) => {
//               let d = frappe.model.add_child(doc, "Dimension Calculation", "custom_dimension_calculation");
//               d.length = row.length;
//               d.width = row.width;
//               d.height = row.height;
//               d.gross_weight = row.gross_weight;
//               d.custom_volume_metric_weight_cm = row.custom_volume_metric_weight_cm;
//               d.box = row.box;
//               d.widht_uom = row.widht_uom;
//               d.length_uom = row.length_uom;
//               d.height_uom = row.height_uom;
//               d.weight = row.weight;
//             });
//           }
...
//             frm.doc.transport_locations.forEach((row) => {
//               let custom_local_transport_location = frappe.model.add_child(doc, "Transport Stops", "custom_local_transport_location");
//               custom_local_transport_location.form_location = row.form_location;
//               custom_local_transport_location.to_location = row.to_location;
//               custom_local_transport_location.exempted_date = row.exempted_date;
//             });
//           }
//           frappe.set_route("Form", "Request for Quotation", doc.name);
//         },
...
//               frappe.call({
//                 method:
//                   "cn_exim.cn_exim.doctype.pickup_request.pickup_request.get_po_all_details",
//                 args: {
//                   po_name: item,
//                 },
//                 callback: function (r) {
...
//           .call({
//             method:
//               "cn_exim.cn_exim.doctype.pickup_request.pickup_request.validate_po_order_qty_to_pickup_qty",
//             args: {
//               po_no: d.po_number,
//               item_code: d.item,
//             },
...
//           (d.length ? d.length : 1) *
//           (d.height ? d.height : 1)) /
//         (frm.doc.type_wise_value ? frm.doc.type_wise_value : 1);
//       d.weight = weight;
//       total_weight += d.weight;
//     });
//     frm.refresh_field("dimension_calculation");
...
//     let purchase_order = [];
//     $.each(frm.doc.purchase_order_details || [], function (i, d) {
//       const isAlreadyInList = purchase_order.some(
//         (item) => item === d.po_number,
//       );
//       if (!isAlreadyInList) {
//         purchase_order.push(d.po_number);
//       }
...
//       frappe.call({
//         method:
//           "cn_exim.cn_exim.doctype.pickup_request.pickup_request.get_items_details",
//         args: {
//           parent: d.po_number,
//           item_name: d.item,
//         },
    ...
    if (frm.doc.docstatus == 1) {
                  ...
                  function () {
                    ...
                    frappe
                      .new_doc("Pre Alert", {
                        ...
                        rfq_number: r.message[0]["name"],
                        // vendor: response.message[0]["supplier"],
                        vendor: frm.doc.name_of_supplier,
                      })
                      .then((doc) => {
                        frappe.set_route("Form", "Pre Alert", doc.name);
                      });
      ...
      frm.add_custom_button(
        ...
        function () {
          ...
          doc.custom_pickup_request = frm.doc.name;
          doc.custom_port_of_loading = frm.doc.pol;
          doc.custom_shipment_date = frm.doc.pickup_date_by || 0;
          doc.custom_eda = frm.doc.custom_estimated_arrival_eta || 0;
          doc.custom_mode_of_shipment = frm.doc.mode_of_shipment;
          doc.custom_pickup_supplier = frm.doc.name_of_supplier;
          doc.custom_shipment_type = frm.doc.type_of_shipments;
          doc.custom_no_of_pkg_units = frm.doc.no_of_boxes;
